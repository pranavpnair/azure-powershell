...
function Test-AliasRecordSet
{
	...
	Assert-ThrowsLike { Get-AzPrivateDnsRecordSet -Name $recordName -ZoneName $zoneName -ResourceGroupName $resourceGroup.ResourceGroupName -RecordType $recordType } "*does not exist*"

	$aliasRecord = $zone | Get-AzPrivateDnsRecordSet -Name $aliasRecordName -RecordType $recordType
	Assert-Null $nonaliasRecord.TargetResourceId
	$aliasRecord | Remove-AzPrivateDnsRecordSet

	Assert-ThrowsLike { Get-AzPrivateDnsRecordSet -Name $aliasRecordName -ZoneName $zoneName -ResourceGroupName $resourceGroup.ResourceGroupName -RecordType $recordType } "*does not exist*"
...
function Test-RecordSetCrudWithPiping
{
	$zoneName = Get-RandomZoneName
	$recordName = getAssetname
    $updatedRecord = TestSetup-CreateResourceGroup | New-AzPrivateDnsZone -Name $zoneName | New-AzPrivateDnsRecordSet -Name $recordName -Ttl 100 -RecordType A -Metadata @{tag1 = "val1"} | Add-AzPrivateDnsRecordConfig -Ipv4Address 13.13.0.13 | Set-AzPrivateDnsRecordSet
...
function Test-RecordSetCrudWithPipingTrimsDotFromZoneName
{
	...
	$createdRecord = $zoneObjectWithDot | New-AzPrivateDnsRecordSet -Name $recordName -Ttl 100 -RecordType A -Metadata @{ tag1 ="val1"}
...
function Test-RecordSetAAAA
{
	...
	$record = $zone | New-AzPrivateDnsRecordSet -Name $recordName -Ttl 100 -RecordType AAAA
...
function Test-RecordSetCNAME
{
	...
	$record = $zone | New-AzPrivateDnsRecordSet -Name $recordName -Ttl 100 -RecordType CNAME
...
function Test-RecordSetMX
{
	...
	$record = $zone | New-AzPrivateDnsRecordSet -Name $recordName -Ttl 100 -RecordType MX
...
function Test-RecordSetTXT
{
	...
	$record = $zone | New-AzPrivateDnsRecordSet -Name $recordName -Ttl 100 -RecordType TXT
...
function Test-RecordSetPTR
{
	...
	$record = $zone | New-AzPrivateDnsRecordSet -Name $recordName -Ttl 100 -RecordType PTR
...
function Test-RecordSetSRV
{
	...
	$record = $zone | New-AzPrivateDnsRecordSet -Name $recordName -Ttl 100 -RecordType SRV
...
function Test-RecordSetNewAlreadyExists
{
	...
	$record = $zone | New-AzPrivateDnsRecordSet -Name $recordName -Ttl 100 -RecordType A | Add-AzPrivateDnsRecordConfig -Ipv4Address 1.2.9.8

	# error the second time
	$message = [System.String]::Format("The Record set {0} exists already and hence cannot be created again.", $recordName);
	Assert-Throws {  $zone | New-AzPrivateDnsRecordSet -Name $recordName -Ttl 212 -RecordType A } $message

	$zone | New-AzPrivateDnsRecordSet -Name $recordName -Ttl 999 -RecordType A -Overwrite -Confirm:$false
...
function Test-RecordSetAddRecordTypeMismatch
{
	$zoneName = Get-RandomZoneName
	$recordName = getAssetname
    $recordSet = TestSetup-CreateResourceGroup | New-AzPrivateDnsZone -Name $zoneName | New-AzPrivateDnsRecordSet -Name $recordName -Ttl 100 -RecordType MX
...
function Test-RecordSetAddTwoCnames
{
    ...
    $recordSet = TestSetup-CreateResourceGroup | New-AzPrivateDnsZone -Name $zoneName | New-AzPrivateDnsRecordSet -Name $recordName -Ttl 100 -RecordType CNAME 
...
function Test-RecordSetEtagMismatch
{
	$zoneName = Get-RandomZoneName
	$recordName = getAssetname
    $recordSet = TestSetup-CreateResourceGroup | New-AzPrivateDnsZone -Name $zoneName | New-AzPrivateDnsRecordSet -Name $recordName -Ttl 100 -RecordType AAAA
